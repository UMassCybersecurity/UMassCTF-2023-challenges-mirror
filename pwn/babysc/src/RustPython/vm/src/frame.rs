...
impl ExecutingFrame<'_> {
    ...
    fn check_access<T: AsRef<str>>(&mut self, vm: &VirtualMachine, target: T) -> bool {
        if self.code.source_path.to_string() == "<embedded>" {
            let varname = vm.ctx.intern_str("capabilities").to_owned();
            let varname: &Py<PyStr> = varname.borrow();
            let capabilities = if let Ok(capabilities) = self.load_global_or_builtin(varname, vm) {
                capabilities
            } else {
                return false;
            };
            let casted: PyRef<PyList> = capabilities.downcast().unwrap();
            for cap in casted.borrow_vec().iter() {
                let cap = if let Some(cap) = cap.payload::<Capability>() {
                    cap.clone()
                } else if let Ok(cap) = Capability::try_from_borrowed_object(vm, cap) {
                    cap
                } else {
                    continue;
                };
                if cap.target == target.as_ref() && validate_capability(&cap) {
                    return true;
                }
            }
            println!("no capability to access '{}'", target.as_ref());
            false
        } else {
            true
        }
    ...
    fn execute_instruction(
        ...
        match instruction {
            bytecode::Instruction::LoadConst { idx } => {
                // println!("Attempting to load constant {:?}!", idx);
            ...
            bytecode::Instruction::ImportName { idx } => {
                ...
                if !self.check_access(vm, name) {
                    return Err(vm.new_lookup_error(format!("no capability to access {name}")));
                }
                self.import(vm, Some(name.to_owned()))
            }
            ...
            bytecode::Instruction::LoadNameAny(idx) => {
                ...
                match result {
                    ...
                    Err(e) if e.fast_isinstance(vm.ctx.exceptions.key_error) => {
                        if !self.check_access(vm, name) {
                            return Err(
                                vm.new_lookup_error(format!("no capability to access {name}"))
                            );
                        }
            ...
            bytecode::Instruction::StoreGlobal(idx) => {
                ...
                self.globals.set_item(name, value, vm)?;
                Ok(None)
            }
            ...
            bytecode::Instruction::LoadMethod { idx } => {
                ...
                if !self.check_access(vm, method_name) {
                    return Err(
                        vm.new_lookup_error(format!("no capability to access {method_name}"))
                    );
                }
    ...
    fn load_attr(&mut self, vm: &VirtualMachine, attr: bytecode::NameIdx) -> FrameResult {
        ...
        if !self.check_access(vm, attr_name) {
            return Err(vm.new_lookup_error(format!("no capability to access {attr_name}")));
        }
